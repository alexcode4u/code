...
class My_Screen(Screen):
    ...
    def start_live(self):
        ...
        t = threading.Thread(target=self.make_live, daemon=True).start()
...
class Test(MDApp):
    ...
    def on_start(self):
        ...
        # p1 = partial(
        #        self.stereo_or_mono, self.root.ids.stereo.ids.check_box, self.root.ids.mono.ids.check_box)
        # print(p1)
        # p2 = partial(
        #        self.stereo_or_mono, self.root.ids.mono.ids.check_box, self.root.ids.stereo.ids.check_box)
        # self.root.ids.stereo.ids.check_box.bind(
        #    on_release=p1)
        # self.root.ids.mono.ids.check_box.bind(
        #    on_release=p2)
    ...
    def cb_handler(self, hlp, item):
        print('mono' if hlp == 'stereo' else 'stereo')
        print(hlp)
